...
def alphabetical
	alpha = liste_journalistes.sort_by(&:downcase)
...
def fifty_shortest
	shortest = liste_journalistes.sort_by(&:length).first(50)
...
def position_epenser
	position = liste_journalistes.index("@epenser")

puts "@epenser est à la position : #{position + 1}"
